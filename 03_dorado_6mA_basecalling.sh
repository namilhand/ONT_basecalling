...
#SBATCH --mem=200G
#SBATCH --array=1-N%1  # Replace N with number of chunks, %1 means process one at a time
#SBATCH -o logs/dorado_chunk_%A_%a.out
#SBATCH -e logs/dorado_chunk_%A_%a.err

# ==============================================================================
# SLURM Job Array Script for Dorado Basecalling with base Modification Detection
# ==============================================================================
...
OUTPUT_BAM="${OUTPUT_DIR}/${CHUNK_BASENAME}_sup_${MODIFIED_BASES}.bam"
...
if [ -f "${OUTPUT_BAM}" ]; then
    ...
    if [ $CHECK_EXIT -eq 0 ] && [ -n "$READ_COUNT" ] ; then
...
if [ -n "${KIT_NAME}" ]; then
    echo "Running with barcode kit: ${KIT_NAME}"
